...
queryFile       = open(os.getcwd() + '\\cacm_stem.query.txt')
...
def queries():
    ...
    lst = queries.split('\n')#re.findall(pattern, queries.replace('\n',' '))
    for query in lst:
        if len(query) > 0:
...
##def getTermWeightInDoc2(term, idf, docDict, docLen):
##    if term not in docDict:
##        termComp = 0.0
##    else:
##    termComp = math.log(decimal.Decimal(docDict[term]))
##
##    numerator = decimal.Decimal((termComp)*idf)
##
##    normalization = decimal.Decimal(0.0)
##    for word in docDict:
##        normalization += pow(decimal.Decimal((math.log(docDict[word]) + 1.0)*idf), 2)
##        
##    return numerator/(decimal.Decimal(math.sqrt(normalization))+ 1)
...
def getDocScore(query, docId):
    ...
    for term in queryDict:
        #docDict = getDocTif(getDoc(term, docId))
        ...
        #wDoc   = getTermWeightInDoc2(term, idf, docDict, docLen)

...
with open('TfIdf_with_Stopping_QueryResults.txt','a+') as queryResults:
...
##            continue
        ...
        rank = 1
        with open('TfIdf_with_Stopping_Query_%d_Result.txt' % i,'a+') as queryResult:
            queryResult.write('query_id Q0 docid rank TfIdf_score system_name')
            for doc in docs:
                queryResults.write('\n%d Q0 %s %d %lf TfIdf_with_Stopping' % (i, doc, rank, docs[doc]))
                queryResult.write('\n%d Q0 %s %d %lf TfIdf_with_Stopping' % (i, doc, rank, docs[doc]))
                if rank == 100:
                    break
                rank += 1
